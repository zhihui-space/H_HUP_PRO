...
import { selectAllOmgPrduFinishPutoutAsPage, updateOmgPrduFinishPutout } from '@/api/orderCompMage/OrderCome'

export default {
  name: "ProductionFlowOrderOrderComeView",
  components: { Table, CommitDialog, ProductionFlowOrderComeContent, OrderConstractProductSel, OrderProductInfoDetails },
  data() {
    return {
      content: {
  ...
  methods: {
    ...
    onButtonClick(param, btnCode, unexParam) {
      ...
      if (param.btnCode === 'OVERCOMEP') { // 质检完成
