...
import ProductionFlowOrderSmtContent from "@/views/productionflow/OrderSmt/subinfo/subinfo"
...
import { selectAllOmgProductionSmtAsPage, updateOmgProductionSmt } from '@/api/orderCompMage/OrderSmt'

export default {
  name: "ProductionFlowOrderRepertoryView",
  components: { Table, CommitDialog, ProductionFlowOrderSmtContent, OrderConstractProductSel, OrderProductInfoDetails },
  data() {
    return {
      content: {
      ...
      columns: [
        ...
        {prop: 'finishNum', label: '累计完成数量', width: '180'},
  ...
  methods: {
    ...
    onButtonClick(param, btnCode, unexParam) {
      ...
      if (param.btnCode === 'OVERSMT') { // SMT全部完成
        if (param.bindData.pkOmgProduSmt) {
          let overParam = {
            pkOmgProduSmt: param.bindData.pkOmgProduSmt,
            billState: '2'
          }
          updateOmgProductionSmt(overParam).then((response) => {
