...
import { selectAllOmgPrduFinishAsPage, insertOmgPrduFinish, deleteOmgPrduFinisByPks, updateOmgPrduFinis, selectAllPrduFinisSubDataAsPage } from '@/api/orderCompMage/OrderIncome'
import { putInStartPutOut } from '@/api/orderCompMage/orderCompMage'

export default {
  name: "ProductionFlowOrderIncomeView",
  components: { Table, CommitDialog, ProductionFlowOrderIncomeContent, OrderConstractProductSel, OrderProductInfoDetails },
  data() {
    return {
      content: {
  ...
  methods: {
    ...
    refreshTableList(constractData) {
      ...
      this.pkProductOrder = constractData.pkProductOrder
      let queryParam = {
    ...
    onButtonClick(param, btnCode, unexParam) {
      ...
      if (param.btnCode === 'BEGINPUTOUT') { // 发起成品出库
      ...
      if (param.btnCode === 'OVERINCOME') { // 入库完成
